...
RUN mkdir -p /drone-sonar-scan-plugin
WORKDIR /drone-sonar-scan-plugin
COPY *.go ./
RUN apk add --no-cache --update git 
RUN GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o ./drone-sonar-scanner
...
COPY --from=build /drone-sonar-scan-plugin/drone-sonar-scanner /bin/
WORKDIR /bin
...
ENTRYPOINT /bin/drone-sonar-scanner
